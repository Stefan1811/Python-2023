...
class ScrabbleGame:
    def __init__(self):
        ...
        self.played_letters = []
        self.played_words = []  # Lista pentru a tine evidenta cuvintelor deja plasate
        self.letter_scores = {
    ...
    def create_board(self):
        for i in range(15):
            for j in range(15):
                ...
                if (i, j) in [(0, 0), (0, 7), (0, 14), (7, 0), (7, 14), (14, 0), (14, 7), (14, 14)]:
                     cell.configure(bg="red")  #triplu cuvant
                elif (i, j) in [(1, 1), (2, 2), (3, 3), (4, 4), (10, 10), (11, 11), (12, 12), (13, 13),
                            (1, 13), (2, 12), (3, 11), (4, 10), (10, 4), (11, 3), (12, 2), (13, 1),(7, 7)]:
                     cell.configure(bg="pink")  #dublu cuvant
                elif (i, j) in [(1, 5), (1, 9), (5, 1), (5, 5), (5, 9), (5, 13), (9, 1), (9, 5), (9, 9), (9, 13),
                            (13, 5), (13, 9)]:
                     cell.configure(bg="blue")  #triplu literă
                elif (i, j) in [(0, 3), (0, 11), (2, 6), (2, 8), (3, 0), (3, 7), (3, 14), (6, 2), (6, 6), (6, 8),
    ...
    def create_rack(self):
        for i, letter in enumerate(self.racks[0]):
    ...
    def rack_tile_clicked(self, col):
        selected_tile = self.racks[0][col]
        if selected_tile.isalpha():
            if selected_tile.isupper():
                self.racks[0][col] = selected_tile.lower()
            elif selected_tile.islower():
                self.racks[0][col] = selected_tile.upper()
            else:
                self.racks[0][col] = selected_tile.title()
            self.played_letters.append(selected_tile)

    def change_letters(self):
        self.clear_rack()
        self.draw_new_tiles(7)
        self.total_letters -= 7
        self.update_rack()
    ...
    def play_word(self):
        ...
        if word and self.validate_word(word) and self.is_word_valid(word):
            if self.place_word_on_board(word):
                ...
                self.score_label.config(text=f"{self.players[0].name}'s Score: {self.players[0].score} | {self.players[1].name}'s Score: {self.players[1].score}")
                messagebox.showinfo("Word Played", f"{self.current_player.name} played the word '{word}' with a score of {score}")
                self.clear_rack()
                self.draw_new_tiles(len(word))
                self.total_letters -= len(word)
                self.remaining_letters_label.config(text=f"Remaining Letters: {self.total_letters}")
                self.update_rack()
                self.played_letters = []
                self.played_words.append(word)  # Adaugă cuvântul în lista cuvintelor deja plasate
                self.switch_player()
            else:
    ...
    def is_word_valid(self, word):
        return all(letter in self.racks[0] + self.played_letters + self.played_words for letter in word)
    ...
    def draw_new_tiles(self, num_tiles):
        self.racks[0] += self.draw_tiles(num_tiles)
    ...
    def reset_rack(self):
        self.total_letters = 100
        self.racks[0] = self.draw_tiles(7)
        self.update_rack()
        self.remaining_letters_label.config(text=f"Remaining Letters: {self.total_letters}")
